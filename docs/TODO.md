...
> 최종 업데이트: 2026-09-06
> 코드베이스 분석 기반 — 실제 구현 상태를 반영합니다. (2026-06-30 → 2026-07-07 종합 고도화 세션, 2026-07-19 rate limiting·다크모드 세션, 2026-08-05 CSP Enforcing 전환, 2026-08-06 소셜 메타태그 강화 착수, 2026-08-07 배포·전채널 검증까지 완료, 2026-08-08 뉴스레터 구독 구현, 2026-08-14 블로그 콘텐츠 전량 삭제·재건 전략 수립 + 전환 퍼널 대시보드 2단계·블로그 하단 CTA 구현, 2026-08-22 영업 지원 트랙(Phase 1.5) 설계·액션플랜 수립, 2026-08-30 전환 퍼널 대시보드 실측 완료 및 Phase 1 공식 종료, 2026-09-02 영업채널 자동 팔로업 전환 결정·구현, 2026-09-03 프로덕션 검증·소개서 v3 확정, 2026-09-06 전체 방향 재검토 — 첫 발송 마감 09/11·신규 기능 동결 게이트·Phase 2 재정의)
...
> **2026-09-06 전체 방향 재검토(사용자 결정 3건)**: ① 1-11 영업이사 첫 링크 발송 마감 **09/11(금)** — 09/10까지 발송이 없으면 사용자가 지인 채널 2~3곳에 직접 발송해 첫 데이터 확보 ② 0-7 뉴스레터·콘텐츠 발송 담당 = **사용자 본인 + AI 비서**(시스템 자동 발송 + 사용자 승인, A-5 결정과 정합 — 액션플랜 0-7 ⬜ 표기는 갱신 누락이었음) ③ Phase 2를 "영업 도구 고도화"로 재정의(4번 참고). **신규 기능 착수 게이트: AX 체크 실응답 5건 + HOT 1건** — 충족 전 E 2차·LLM·Calendly·Broadcasts 코드 착수 금지. 근거·경위: 기획서 3-B·7-C, 노션 액션 DB(1-11·2단계·3단계·E 2차 메모)
...
- 🚧 **1단계 영업 가동 (08/27~09/05)** — 코드 구현 완료(2026-08-25): `catalog.ts`+`summarize.ts`(등급 규칙·Q3 매핑·Q5 분기, Vitest 13개) + `src/actions/ax-check.ts`(검증·rate limit·저장·옵트인 구독·메일 2통, Vitest 25개) + `/ax-check`(8문항 전부 선택지, 3분, `?ref=` 보존) + `/ax-check/result/[token]` + `/admin/leads`(HOT/WARM/COLD·상태·메모·CSV·삭제) + GA4 `ax_check_submit`(`source`=ref) 이벤트·퍼널 패널 병기 + `/privacy`·`CONTENT_GUIDE.md` 17번. 질문지 문구는 여전히 v1-draft(0-3 인터뷰 결과 대기, `catalog.ts` 상단 주석 참고). ⬜ 남은 것: 영업이사 실기기 테스트, **첫 링크 발송** — 최초 목표(08/27~09/05)에서 지연 중. 09/02 1차 실기기 테스트에서 수동 발송 워크플로우 문제를 확인해 자동 팔로업 전환을 결정(아래 항목), 전환 반영분(C단계) 위에서 재테스트 후 발송 예정. `prisma migrate deploy`·Playwright E2E는 2026-08-26~27 완료.
  ...
  **2026-09-06 재실행 완료(사용자 확인 후 진행)**: 1차 실행 실패 — `e2e/ax-check.spec.ts`의 Q3(반복 업무) 체크박스 라벨이 09-01 v2 카탈로그 반영 전 문구("제안서·견적서 작성" 등)로 남아있어 발생한 테스트 코드 노후화(실제 제출 전 실패라 DB·메일 부작용 없음). 라벨을 현재 `catalog.ts` v2 문구("견적·내역서·투찰 서류 작성"·"나라장터 입찰 공고 탐색·적격심사 서류 준비")로 수정(커밋 예정) 후 재실행 → **골든패스 PASS**(제출→결과 화면 문구 정상, 관리자 로그인 확인부는 로컬 `E2E_ADMIN_EMAIL` 미설정으로 정상 skip). 이 실행으로 생성된 프로덕션 테스트 리드는 `/admin/leads`에서 삭제 필요
- 🚧 **2단계 보강 (09/08~09/26) — 2026-09-02 재편**: 자동 팔로업 시퀀스(T0 즉시 요약 메일·T1 D+2 상세 진단 자동 발송·관리자 보류/즉시발송·영업일 계산·Vercel Cron) **구현 완료(위 항목 참고, 계획보다 이르게 09/02~03 마무리)**. 남은 것: 완료율 점검(20% 미만이면 문항 8→6), `/solutions` 단일 오퍼 재편(+`/admin/solutions` 폼·가이드 갱신), `/about` 레퍼런스 중심 축약, 블로그 1편 — 전부 미착수. LLM 상세 진단서는 자동 발송 파이프라인 위의 후속 개선(응답 20건 이상)으로 이동
  - **2026-09-06 진행 방식 확정**: 완료율 점검은 실응답 5건 이후에만(데이터 없으면 09/08 착수 대신 발송 독려 우선). `/solutions` 재편은 CMS 텍스트 범위 우선(`PageContent` 구조 변경 최소화), `/about` 축약은 CMS만으로(코드 없음). 블로그 1편은 08-31 영업이사 인터뷰 "자주 받는 질문" 기반으로 사용자 + AI 비서가 작성(소싱 정책 16번). ✅ Vercel env(`SALES_NOTIFY_EMAIL` 변경·`SALES_NOTIFY_CC_EMAIL` 신규) 등록·재배포·테스트 리드 1건 제출로 확인 완료(2026-09-06 저녁, 사용자 확인 — 영업이사 메일함 To·기술이사 메일함 Cc 양쪽 정상 수신). ✅ Playwright `ax-check.spec.ts` 재실행 완료(2026-09-06, 위 1-B 항목 참고 — 테스트 코드 노후화 수정 후 PASS)
...
> — **소셜 메타태그 강화는 2026-08-07 완료**, **뉴스레터 구독은 2026-08-08 완료**, **전환 퍼널 대시보드는 1단계(이벤트 태깅) 2026-08-08 완료, 2단계(시각화 UI) 구현 2026-08-14 완료, 실 데이터 검증·배포 2026-08-30 완료** — **Phase 1 공식 종료.**
> **Phase 2 재정의(2026-09-06 사용자 결정)**: 8/3 원안(CMS 구조 편집·댓글/반응·관리자 다크모드)은 "홍보팀이 쓰는 마케팅 플랫폼"을 전제로 했으나, 실제 리드 채널이 영업이사 아웃바운드로 바뀌고 4인 분산 체제에 별도 홍보팀이 없어 **"영업 도구 고도화"로 재정의**. 착수는 Phase 1.5 종료 후(11월 이후), 우선순위는 Phase 1.5 실응답 데이터로 재확정. 상세: 기획서 3-B·4번 Phase 2·7-C

### 중기 — Phase 2 후보 (영업 도구 고도화, 11월 이후)

- 💡 **리드 알림·처리 편의** — 카카오톡 실시간 알림, `/admin/leads` 필드 확장(담당자·다음 액션·통화 메모) — 2026-08-31 영업이사 인터뷰 범위 밖 요구
- 💡 **응답 데이터 기반 개선** — 완료율·등급 분포에 따른 문항 조정, LLM 상세 진단서(응답 20건 이후), 업종 확장(건설 등)
- 💡 **콘텐츠 파생 파이프라인** — 진단 응답 익명 통계 → 블로그/FAQ 자동 초안(AI 비서), T2 너처 메일 소재화
- 💡 **Notion-세일즈 CRM 연계 재검토** — `/admin/leads`가 실제 파이프라인이 되면 노션 액션 DB 연동 필요성 재평가

### 아이디어 주차장 (당장 스코프에 넣지 않음)

- 💡 **댓글/반응 기능** — 블로그 좋아요·댓글 (2026-09-06 Phase 2에서 이동)
- 💡 **관리자 패널 다크모드** — 공개 페이지는 완료(위 1번 참고), `/admin/**`은 범위 밖 (2026-09-06 Phase 2에서 이동)
- 💡 **CMS 구조 편집 확장** — 홈/소개/솔루션 카드 개수·순서 재설계 (2026-09-06 Phase 2에서 이동)
- 💡 **회원 전용 콘텐츠 영역** — 로그인 회원 전용 심화 자료·리포트
- 💡 **다국어(i18n) 지원** — 국내 세그먼트 첫 계약 전까지 착수하지 않음
- 💡 **예약/미팅 시스템** — → **2026-08-22 Phase 1.5 3단계로 이동**(위 1-B 참고)
