...
- 완료 기준(Definition of Done): 전환 퍼널 분석 대시보드가 프로덕션에 배포되고, 문의 전환율·소스별 전환율이 관리자 대시보드에서 정량적으로 확인 가능해지면 Phase 1 종료 — **2026-08-30 DoD 충족 확인, Phase 1 공식 종료**. Phase 2는 Phase 1.5 종료 후(11월 이후)이며 **2026-09-06 "영업 도구 고도화"로 재정의**(원안 CMS 구조 편집·댓글/반응·관리자 다크모드는 아이디어 주차장으로 이동, 기획서 3-B·4번·7-C 참고)
- **2026-09-06 전체 방향 재검토 결정**: 사이트의 역할은 "리드 육성 플랫폼"이 아니라 **"영업이사의 도구 + 리드 구조화 장치"**. ① 1-11 첫 링크 발송 마감 09/11(09/10까지 없으면 사용자 직접 2~3곳) ② 뉴스레터·콘텐츠 발송 담당 = 사용자 + AI 비서(시스템 자동 + 승인) ③ **신규 기능 착수 게이트: AX 체크 실응답 5건 + HOT 1건** — 충족 전 E 2차(T2 너처·Calendly·LLM·Broadcasts)·3단계 코드 착수 금지. 콘텐츠는 영업에서 소재를 받아오는 구조(인터뷰 Q&A → FAQ·블로그, 응답 통계 → 다음 글)
...
- [ ] **Phase 1.5 영업 지원 트랙** — ✅ 0단계 준비(인터뷰·질문지 v2·소개서 4~5p 게시·마이그레이션·EDITOR/ref) → ✅ 1단계 `/ax-check`+`/admin/leads` 배포·실기기 테스트·매뉴얼 전달(09/06) → ⬜ **1-11 첫 링크 발송(~09/11)** → ✅ 2단계 핵심 자동 팔로업 구현·검증(09/03, 계획보다 선행) → ⬜ 2단계 잔여 `/solutions` 재편·`/about` 축약·블로그 1편·완료율 점검(실응답 5건 후, ~09/26) → ⬜ 3단계 뉴스레터 1호·Calendly·업종 확장 결정·종료 보고(09/29~10/31, 게이트: 실응답 5건 + HOT 1건)
...
- Phase 2 후보(2026-09-06 재정의 "영업 도구 고도화", 11월 이후, 실응답 데이터로 우선순위 재확정): 카카오톡 실시간 리드 알림, `/admin/leads` 필드 확장, 응답 데이터 기반 문항 조정·LLM 상세본(20건 이후)·업종 확장(건설 등), 콘텐츠 파생 파이프라인(응답 통계 → 블로그/FAQ 초안), Notion-세일즈 CRM 연계 재검토
- 아이디어 주차장(당장 스코프에 넣지 않음): CMS 구조 편집 확장(카드 개수·순서), 블로그 댓글/반응 기능, 관리자 패널(`/admin/**`) 다크모드(이상 3건 2026-09-06 Phase 2에서 이동), 회원 전용 콘텐츠 영역, 다국어(i18n) 지원. 예약/미팅 시스템은 Phase 1.5 3단계로 이동
